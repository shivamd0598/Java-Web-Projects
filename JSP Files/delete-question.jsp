    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item ">
       ...
       <li class="nav-item">
        ...
        <a class="nav-link" href="delete-question.jsp">Delete Questions<span class="sr-only">(current)</span></a>
      </li>
            <li class="nav-item">
...
<br>
     ...
     <div class="container">
          ...
          <strong><center><h1 style="color: red; background-color: gray;">DELETE QUESTION</h1></center></strong>
         
         
         
    
         
        <form action="delete-question.jsp" method="post">
     ...
     <tr>
      <td>   <strong> Course: </strong></td> <td> <select name="course" id="gend"> 
       ...
       <%
        ...
        PreparedStatement ps1= con1.prepareStatement("select * from courses");
        ResultSet res1=ps1.executeQuery();
        while(res1.next())
        {
   
       %>
       <option name=<%=res1.getString("course_name") %>> <%=res1.getString("course_name") %> </option> 
...
<br>
  ...
  <thead>
    <tr id="tr">
      ...
      <th scope="col">Delete</th>
        ...
        {
        int i=0;
        String topic=request.getParameter("course"); 
        if(topic.equals(" "))
        {
        	   out.println("<script type=\"text/javascript\">");
        	   out.println("alert('Please select the course!!!');");
        	   out.println("window.location='delete-question.jsp'");
        	   out.println("</script>");	
        }
        else
        {
        ...
        PreparedStatement ps= con.prepareStatement("select * from question_ans inner join question using(qid) where topic=?");
  ...
  <tbody>
    ...
    <tr id="tr">
      <th scope="row"><%=i %></th>
      <td> <%=res.getString("ques_name")   %>  </td>
      <td> <%=res.getString("option1")   %></td>
      <td> <%=res.getString("option2")   %></td>
      <td> <%=res.getString("option3")   %></td>
      <td> <%=res.getString("option4")   %></td>
      <td> <%=res.getString("ans")   %></td>
      <td>
      <form action="DeleteQuestionServlet" method="post">
     <button type="submit" class="btn btn-primary" id="delete-btn" name="del-btn" value=<%=res.getString("qid") %> >DELETE </button>
     </form>
